...
team_t team = {
    ...
    "Seeam T. Khan",     /* First member full name */
    "skhan96@gwu.edu",  /* First member email address */
...
/* below are the main computations for your implementation of the rotate. Any changes in implementation will go here or the other functions it may call */
	...
	int dimV = dim*(dim-1);
	int k, l, dim2, d;
	for (i = 0; i < dim; i += 16){//do calculation in blocks of 16x16
		for (j = 0; j < dim; j += 16){
			for(k = i; k < i + 16; k++){
				dim2 = k*dim;
				d = dimV + k;
				for(l = j; l < j + 16; l++){
						dst[d - l*dim] = src[dim2 + l];
				}
			}
		}
	}
...
/* below are the main computations for your implementation of the smooth function. Any changes in implementation will go here or the other functiosn it calls */
	...
	for (i = 0; i < dim; i++){
		for (j = 0; j < dim; j++){
			...
			assign_sum_to_pixel(&current_pixel, sum);//answer from avg() in current_pixel
			dst[((i)*(dim)+(j))] = current_pixel; //set pixel to same spot in dst
		}
	}	
